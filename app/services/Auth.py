...
class AuthService:
    ...
    def get_user(self,email,password):
        try:
            user = self.user_model.get_user({"email": email })
            if not user:
                return {
                    "success": False,
                    "data": None,
    ...
    def signup(self, user_data) -> dict:
        ...
        """
        try:
            existing_user = self.user_model.get_user({"email": user_data.email})
            if existing_user:
                return {
                "success": False,
                "data": None,
            ...
            user_id = self.user_model.create_user(user_data_dict)
            # user_data_dict["_id"] = str(user_id) 
            # user = UserSchema(**user_data_dict)
            response = {
                "success": True,
    ...
    def send_otp_email(self, email: str):
        try:
            ...
            self.otp_model.save_otp(email, otp)
    ...
    def verify_otp_reset_password(self, email: str, input_otp: int, new_password: str):
        """Verify OTP and update the user's password"""
        try:
            otp_record = self.otp_model.get_otp(email)
            if not otp_record:
                return {
                        "success": False,
                        "data": None,
            ...
            self.user_model.update_password(email, new_hashed_password)
            self.otp_model.delete_otp(email)
            return  {
                        "success": True,
    ...
    def get_all_users(self, page: int = 1, limit: int = 10):
        try:
            filters = {}
            limit = limit
            total = self.user_model.get_documents_count(filters)

            total_pages = (total + limit - 1) // limit
            number_to_skip = (page - 1) * limit
            users = self.user_model.get_users(filters, number_to_skip, limit)
    ...
    def delete_user(self, user_id: str):
        try:
            deleted = self.user_model.delete_user(user_id)
            if deleted:
    ...
    def update_user(self, user_id: str, update_data: dict) -> dict:
        ...
        try:
            ...
            updated = self.user_model.update_user(user_id, update_data)
            if not updated:
