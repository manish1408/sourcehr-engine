...
class QuizService:
    ...
    async def create_quiz(self, data: QuizCreate, user_id) -> dict:
        try:
            quiz_data = data.model_dump(exclude_unset=True)
            quiz_data["createdBy"]=user_id
            inserted_id = await self.quiz_model.create_quiz(quiz_data)
            return {
                "success": True,
    ...
    async def list_quizzes(self, page: int = 1, limit: int = 10, user_id: str = None) -> dict:
        try:
            limit=limit
            total = await self.quiz_model.collection.count_documents({"createdBy":user_id})
            total_pages = (total + limit - 1) // limit
            number_to_skip = (page - 1) * limit
            quizzes = await self.quiz_model.list_quizzes({"createdBy":user_id}, number_to_skip, limit)
            return {
                "success": True,
                "data": {
                    "quizzes": quizzes,
    ...
    async def get_quiz_by_id(self, quiz_id: str) -> dict:
        try:
    ...
    async def update_quiz(self, quiz_id: str, data: dict) -> dict:
        try:
            if not ObjectId.is_valid(quiz_id):
                raise HTTPException(status_code=400, detail="Invalid Quiz ID")
            updated = await self.quiz_model.update_quiz(quiz_id, data)
            if not updated:
    ...
    async def delete_quiz(self, quiz_id: str) -> dict:
        try:
            if not ObjectId.is_valid(quiz_id):
                raise HTTPException(status_code=400, detail="Invalid Quiz ID")
            data = await self.quiz_model.delete_quiz(quiz_id)
            return {
                "success": True,
    ...
    async def add_question(self, quiz_id: str, question_data: QuestionCreate):
        try:
            data = await self.quiz_model.add_question(quiz_id, question_data.model_dump())
            return {
                "success": True,
    ...
    async def update_question(self, quiz_id: str, question_id: str, updates: QuestionUpdate):
        try:
            data = await self.quiz_model.update_question(
    ...
    async def remove_question(self, quiz_id: str, question_id: str):
        try:           
            data = await self.quiz_model.remove_question(quiz_id, question_id)
            return {
                "success": True,
