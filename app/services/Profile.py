...
class ProfileService:
    ...
    def change_profile_picture(self, user_id: str, file):
        ...
        try:
            ...
            existing_profile = self.profile_model.get_profile(ObjectId(user_id))
            ...
            self.profile_model.update_profile(ObjectId(user_id), new_filename)       
            return {"success": True, "data": f"Profile picture updated successfully with name {new_filename}"}
        except Exception as e:
            return {"success": False, "data": None, "error": str(e)}
        
    def update_user_info(self, user_id: str, data: dict):
        try:
            if not data:
                raise Exception("No data provided for update")
            updated = self.user_model.update_user(user_id, data)
            if not updated:
    ...
    def get_current_user(self, token: str):
        try:
            ...
            user_doc = self.user_model.collection.find_one({
                "email": email,
    ...
    def save_onboarding(self, user_id: str,onboardingStep:int):
        try:
            if onboardingStep==3:
                
                resp=self.user_model.update_user(user_id,{"onboardingStep":onboardingStep,"isOnboarded":True})
            else:
                resp=self.user_model.update_user(user_id,{"onboardingStep":onboardingStep,"isOnboarded":False})
