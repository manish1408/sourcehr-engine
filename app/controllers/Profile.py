...
def update_user_info(
    ...
    try:
        ...
        data = service.update_user_info(user_id,body.model_dump(exclude_unset=True))
...
def get_me(
    ...
    try:
        ...
        result = profile_service.get_current_user(token)
...
def save_onboarding(onboardingStep:int,profile_service: ProfileService = Depends(get_profile_service),jwt_payload: dict = Depends(jwt_validator)):
    try:
        user_id = jwt_payload.get("id")
        data = profile_service.save_onboarding(user_id,onboardingStep)
        return Utils.create_response(data["data"], data["success"], data.get("error", ""))
    except Exception as e:
        raise HTTPException(status_code=400, detail={"data": None, "error":str(e),"success": False}) 
