...
class DocumentModel:
    ...
    def get_document(self, filters: dict) -> Optional[DocumentsSchema]:
        """
    ...
    def get_documents(self, filters: dict = {}, skip: int = 0, limit: int = 10) -> List[DocumentsSchema]:
        """
        Retrieve a list of documents matching the given filters with pagination.
        """
        cursor = self.collection.find(filters).skip(skip).limit(limit)
        return [DocumentsSchema(**doc) for doc in cursor]
    ...
    def get_documents_with_projection(self, filters: dict = {}, skip: int = 0, limit: int = 10, fields: List[str] = None) -> List[dict]:
        """
    ...
    def create_document(self, data: dict) -> PyObjectId:
        """
    ...
    def push_vector_id(self, document_id: str, vector_id: str) -> bool:
        """
        Push a new vector ID to the document's VectorDocId array.
        """
        filters = {"_id": ObjectId(document_id)}
        result = self.collection.update_one(filters, {"$push": {"vectorDocId": vector_id}})
        return result.modified_count > 0
    ...
    def delete_many_document(self, filter:dict) -> bool:
        """
        Permanently delete a document from the database.
        """
