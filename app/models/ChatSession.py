...
class ChatSessionModel:
    ...
    async def create_session(self, data: dict) -> PyObjectId:
        """
    ...
    async def get_session(self, filters: dict) -> Optional[dict]:
    ...
    async def get_session_with_projection(self, filters: dict = {}, skip: int = 0, limit: int = 5, fields: List[str] = None) -> List[dict]:
        """
    ...
    async def get_sessions(self, filters: dict = {}, skip: int = 0, limit: int = 10) -> List[ChatSessionSchema]:
        cursor = self.collection.find(filters).skip(skip).limit(limit).sort("createdOn",-1)
        results = await cursor.to_list(length=limit)
        return [ChatSessionSchema(**doc) for doc in results]
    ...
    async def update_message_with_message_id(self, session_id: str, message_id: str, message: str, citations: list):
    ...
    async def update_message_sentiment_with_message_id(self, session_id: str, message_id: str, sentiment: str):
        parent_id = ObjectId(session_id)
        message_id = ObjectId(message_id)
        result = await self.collection.update_one(
            { "_id": parent_id, "messages._id": message_id },
    ...
    async def get_sessions_count(self, filters: dict) -> int:
        """
